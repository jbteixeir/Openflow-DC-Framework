...
class ERCSStatsExport(object):
    ...
    def __init__(self, inithandler = None, topology = None, stats = None, vm_manager = None):
    ...
    def export_stats_switches(self):
        while (1):
            for dpid in self.coredpid:
                ratio = self.vm_manager.getSwitchRatio(dpid)
    ...
    def export_stats_links(self):
        while(1):
            for (dpid,port_id, dst_dpid, dst_port) in self.corelinks :
                ratio = self.vm_manager.getLinkRatio(dpid,port_id)
    ...
    def export_stats_hosts(self):
        while(1):
            ...
            for host in self.topology.hosts:
                if self.vm_manager.vms_allocated.has_key(host):
                    self.host_stats_file.write(str(len(self.vm_manager.vms_allocated[host]))+";")
                else:
